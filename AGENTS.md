...
## Ponytail — лень как инженерная дисциплина

Правило действует на любую кодовую задачу: писать меньше кода, чем просят, но ровно столько, чтобы работало. Полные правила — `~/repo/ponytail/AGENTS.md`, снимок пакета `356918e`.

Лестница: остановись на первой ступени, которая держит.

1. Нужно ли это вообще (YAGNI).
2. Есть ли это уже в кодовой базе — переиспользуй, не переписывай.
3. Умеет ли это стандартная библиотека.
4. Есть ли штатная возможность платформы.
5. Решает ли это уже установленная зависимость.
6. Влезает ли в одну строку.
7. Только теперь — минимум кода, который работает.

Лестница идёт **после** понимания задачи, не вместо: прочитай задачу и код, который трогаешь, проследи реальный путь целиком, потом поднимайся.

Правки багов — по корневой причине, не по симптому: почини общую функцию один раз, а не каждый вызов по отдельности.

Запрещено: незаказанные абстракции, новая зависимость там, где можно без неё, шаблонный код, о котором никто не просил. Удаление лучше добавления, скучное лучше умного.

**Нельзя лениться о:** понимании задачи, проверке входных данных на границах доверия, обработке ошибок с потерей данных, безопасности, доступности, явно запрошенном. Нетривиальная логика оставляет одну запускаемую проверку — самую маленькую, которая падает, если логика сломана.

Осознанное упрощение с известным потолком помечать комментарием `ponytail:` с указанием потолка и пути развития.

Агенты со скиллами (Claude Code, Codex, Cursor) получают то же правило через свои каналы; здесь оно для тех, кто читает контракт репозитория (Antigravity и прочие).
