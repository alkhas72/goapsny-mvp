...
describe('MapLibreMap', () => {
  ...
  it('fits to Abkhazia bounds on initial mount and does not prompt geolocation', () => {
    ...
    // Bounds array form: [[swLng, swLat], [neLng, neLat]] — Abkhazia bbox.
    const call = mockMapInstance.fitBounds.mock.calls[0][0] as number[][];
    const [[swLng, swLat], [neLng, neLat]] = call;
    expect(swLat).toBeGreaterThan(42);
    expect(neLat).toBeLessThan(44);
    expect(swLng).toBeGreaterThan(39);
    expect(neLng).toBeLessThan(42);

    // Critical: initial public load must NOT trigger any permission prompt.
